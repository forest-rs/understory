...
- A first `understory_display` crate now exists in this repository with a
  deliberately small retained display list and rect/rounded-rect ops.
...
- How much text specialization to bake in (stick with `GlyphRun` vs a higher-level paragraph op).
...
- Define a small `Painter` or `PaintScene`-style trait that writes into `DisplayListBuilder`, to be shared by higher layers (presentation node, SVG, widgets).
